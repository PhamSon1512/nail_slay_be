...
**_entity_** , **_value object_** ...), cho phép nhóm phát triển và chuyên gia nghiệp vụ (domain
...
Application, Infrastructure). **Tầng Domain** chứa các **_Entity, Value Object, Domain
Service..._** thuần túy, không dính dáng framework. **Tầng Application** điều phối các use
...
này. Ví dụ: **Repository patter** n (kho lưu trữ) giúp trừu tượng hóa việc truy xuất và lưu trữ
...
_// Lưu ID đơn hàng vào session_
$request->session()->put('currentOrderId', $orderId);
_// Trả về phản hồi (view) cho người dùng
// return view('orders.success', ['orderId' => $orderId]);_
}
...
toán mà phần mềm giải quyết. Việc xây dựng domain model thường liên quan đến các **_Entity,
Value Object, Domain Service_** ,... tuân theo tư tưởng của **_Domain-Driven Design_**.
...
_// Kiểm tra và gán giá trị_
if ($ebookId <= 0 ) {
throw new \InvalidArgumentException('ID ebook phải > 0');
}
if (!filter*var($emailAddress, FILTER_VALIDATE_EMAIL)) {
...
*// Các phương thức getter cho thuộc tính\_
...
_// (Có thể thêm các logic khác, ví dụ tính thuế, áp mã giảm giá, v.v.)_
}
...
thiết kế: **_giả định có sẵn một thành phần mình cần rồi định nghĩa nó, sau đó mới lo cài đặt_**.
...
_* Tìm Order theo ID, ném ngoại lệ nếu không tìm thấy
*/_
public function getById(int $orderId): Order;
}
_(Ghi chú: Nếu dùng UUID tự sinh cho Order thì có thể không cần trả về int ID. Nhưng ở đây giả định
dùng autoincrement ID do DB cấp.)_
...
use App\Domain\Order; _// Giả sử Order nằm trong App\Domain_
...
public function save(Order $order): int
{
_// Chuyển đổi Order entity thành dữ liệu để lưu_
$data = [
...
_// Thực hiện insert vào database_
...
_// Tạo lại Order entity từ dữ liệu DB_
return new Order(
$record->ebook_id,

$record->email,
$record->quantity,
(int) ($record->amount / $record->quantity) _// pricePerUnit =
amount / quantity_
);
}
}
...
$quantity = (int) $request->input('quantity');
$email = $request->input('email_address');
_// Lấy giá e-book từ repository (hạ tầng truy vấn DB)_
$price = $this->ebookRepo->getPriceById($ebookId);
try {
_// Tạo Order entity (domain)_
$order = new Order($ebookId, $email, $quantity, $price);
} catch (\Exception $e) {
_// Dữ liệu không hợp lệ → trả về lỗi cho người dùng_
return response(
...
_// Lưu Order thông qua repository_
$orderId = $this->orderRepo->save($order);
_// Lưu ID đơn hàng vào session (hạ tầng)_
$request->session()->put('currentOrderId', $orderId);
_// Trả về response (ví dụ redirect tới trang hiển thị đơn hàng vừa
đặt)_
...
$request->get('items'),
$request->get('email') *// ... các tham số khác );\_
...
_// ... có thể thêm EmailService, PaymentService nếu cần_
public function __construct(
OrderRepository $orderRepo,
ProductRepository $prodRepo _/*,...*/_
) {
$this->orderRepo = $orderRepo;
...
_// (Gửi email xác nhận nếu cần, có thể phát domain event hoặc gọi
EmailService)_
return $orderId;
}
}
...
├── Domain/
│ └── Order/
│ ├── Order.php _# (Entity)_
│ ├── ValueObjects/ _# (Các đối tượng giá trị)_
│ ├── Event/ _# (Domain Events)_
│ └── Repository/
│ └── OrderRepository.php _# (Port: Persistence)_
│
├── Application/
│ └── Order/
│ ├── PlaceOrderHandler.php _# (Use case logic)_
│ └── Port/
│ └── OrderNotification.php _# (Port: Notification)_
│
└── Infrastructure/
├── Order/
...
│ │ ├── Http/
│ │ │ └── OrderController.php _# (Adapter for UI port -
Web)_
│ │ └── Console/
│ │ └── ImportOrderCommand.php _# (Adapter for UI port -
CLI, giả sử có)_
│ │
│ ├── Persistence/
│ │ ├── EloquentOrderRepository.php _# (Adapter for
Persistence port - MySQL)_
│ │ └── InMemoryOrderRepository.php _# (Adapter for
Persistence port - Test)_
│ │
│ └── Notifications/
│ ├── EmailOrderNotification.php _# (Adapter for
Notification port - Email)_
│ └── LogOrderNotification.php _# (Adapter for
Notification port - Fake)_
│
└── Providers/
└── HexagonBindings.php _# (Đăng ký DI cho các
port)_
...
public function handle(PlaceOrderCommand $cmd): OrderId
{
_// ... [tạo Order, thêm sản phẩm, confirm như trước] ..._
$this->orderRepo->save($order);
_// Gửi thông báo qua cổng Notification (không biết chi tiết gửi
email)_
...
{
_// Sử dụng Mailable hoặc Mail facade để gửi email\_
...
new InMemoryProductRepository($fakeProducts), _// Giả lập danh sách
product_
new LogOrderNotification()
);
$command = new PlaceOrderCommand(
$customerId,
[ _/* ... danh sách sản phẩm ... */_ ]
);

$orderId = $handler->handle($command);
_// assert: kiểm tra đơn hàng đã lưu trong InMemoryOrderRepository
